...
| [`test_cache_root.py`](../backend/tests/test_cache_root.py) | All four cache dirs (`basic`/`diff`/`openvino`/`video`) are direct children of `CACHE_BASE_DIR`; `CACHE_DIR` env var overrides the root, else `DATA_DIR/cache`; OpenVINO cache key is class-order-insensitive and confidence-sensitive | [`visualization-modes.md`](visualization-modes.md#cache-management), [`deployment.md`](deployment.md) |
...
| [`test_google_oauth.py`](../backend/tests/test_google_oauth.py) | `get_access_token()`: cached token reused while >60 s valid, refresh via token endpoint otherwise, `invalid_grant` → disconnect + `NotConnected` (httpx mocked) | [`google-integration.md`](google-integration.md) |
| [`test_claude_provider.py`](../backend/tests/test_claude_provider.py) | Claude batch request layout (base64 JPEG blocks, prompt text last), structured response parsed and saved per file, unparseable → nothing saved (Anthropic client mocked) | [`ai-analysis.md`](ai-analysis.md) |
| [`test_gemini_provider.py`](../backend/tests/test_gemini_provider.py) | Gemini batch request layout (prompt first, then images), missing `usage_metadata` → zero tokens, structured response saved per file (google-genai mocked) | [`ai-analysis.md`](ai-analysis.md) |
