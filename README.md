...
| [`docs/database.md`](docs/database.md) | SQLite schema: `cameras`, `files`, `thumbnails`, `ai_analysis`, `object_detection`, `video_previews`, `tasks`, `tuning_sessions` tables, cascades, data flow |
...
| [`docs/visualization-modes.md`](docs/visualization-modes.md) | All 5 view modes: Normal, Motion highlight, AI description (Gemini), AI description (Claude), Object detection (local) — plus the shared cache layout |
...
| [`docs/home-assistant-addon.md`](docs/home-assistant-addon.md) | HA add-on: single-container packaging, ingress flow, run.sh entrypoint, release CI (workflow_dispatch + manual tag), minimum HA setup |
...
- **Demo Camera** — points to `backend/demo_camera/` (bundled sample images, auto-scanned on first launch)
- **My Camera** — placeholder; shows HA setup instructions with clickable links until configured

**Camera Root** (the base folder for all camera paths) is configured in **Tools → Cameras → Camera Root**.
In the HA add-on this defaults to `/media`; in local dev set the `CAMERA_ROOT` env var.
The in-app setting (persisted to `server_config.json`) takes priority over the env var.
